...
: m_ipo_as_force(false),
  ...
  m_ipo_start_euler(MT_Vector3(0.0, 0.0, 0.0)),
...
bool KX_IpoSGController::Update(double currentTime)
{
  ...
  if (m_ipo_channels_active[OB_LOC_X]  || m_ipo_channels_active[OB_LOC_Y]  || m_ipo_channels_active[OB_LOC_Z] ||
    ...
    if (m_ipo_as_force == true) {
      if (m_game_object && m_node && m_game_object->GetPhysicsController()) {
        MT_Vector3 vec = m_ipo_local ? m_node->GetWorldOrientation() * m_ipo_xform.GetPosition() : m_ipo_xform.GetPosition();
      ...
      MT_Vector3 newPosition = MT_Vector3(0.0, 0.0, 0.0);
  ...
  if (m_ipo_channels_active[OB_SIZE_X] || m_ipo_channels_active[OB_SIZE_Y] || m_ipo_channels_active[OB_SIZE_Z] ||
    ...
    MT_Vector3 newScale = MT_Vector3(1.0, 1.0, 1.0);
    if (!m_ipo_add)
